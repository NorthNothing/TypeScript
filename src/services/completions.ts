...
namespace ts.Completions {
    export enum SortText {
        ...
        CurrentFileScope = "2",
        GlobalsOrKeywords = "3",
        AutoImportSuggestions = "4",
        JavascriptIdentifiers = "5"
    }
    ...
    function getCompletionData(
        ...
        function getGlobalCompletions(): void {
            ...
            for (let node = scopeNode; !!node; node = node.parent) {
                if (node.locals && (!isSourceFile(node) || isExternalOrCommonJsModule(node))) {
                    locals = node.locals;
                    break;
                }
            }
            ...
            for (const symbol of symbols) {
                if (!locals || locals.get(symbol.escapedName) !== symbol) {
                    symbolToSortTextMap[getSymbolId(symbol)] = some(symbol.declarations, d => d.getSourceFile() === sourceFile) ?
                        SortText.CurrentFileScope :
